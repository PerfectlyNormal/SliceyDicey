...
public class HeatshrinkDecoder
{
    ...
    public HeatshrinkDecoder(int inputBufferSize, int windowBits, int lookaheadBits)
    {
        ...
        if (Constants.EnableLogging)
            Console.WriteLine(
                $"-- allocated decoder with buffer size of {bufferSz} ({1 << windowBits} + {_inputBufferSize})");
    }
    ...
    public DecoderPollResult Poll(byte[] buffer, int offset, int size, out int outputSize)
    {
        ...
        while (true)
        {
            if (Constants.EnableLogging)
                Console.WriteLine(
                    $"-- poll, state is {Convert.ToInt32(_state)} ({StateNames[_state]}), input_size {_inputSize}");
        ...
        for (var i = 0; i < count; i++)
        {
            if (_bitIndex == 0x00)
            {
                if (_inputSize == 0)
                {
                    if (Constants.EnableLogging)
                        Console.WriteLine(
                            $"  -- out of bits, suspending w/ accumulator of {accumulator} (0x{accumulator:x2})");
                    return -1;
                }
